...
By default, MoonHub blocks the following dangerous commands:
...
All configuration options can be overridden via environment variables with the format `MOONHUB_TOOLS_<SECTION>_<KEY>`:

For example:

- `MOONHUB_TOOLS_WEB_BRAVE_ENABLED=true`
- `MOONHUB_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `MOONHUB_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`
- `MOONHUB_TOOLS_MCP_ENABLED=true`
