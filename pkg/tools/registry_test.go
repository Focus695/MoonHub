...
func TestToolRegistry_ExecuteWithContext_InjectsToolContext(t *testing.T) {
	...
	r.ExecuteWithContext(context.Background(), "ctx_tool", nil, "telegram", "chat-42", "deleg:sk1", nil)
	...
	if got := DelegationUserID(ct.lastCtx); got != "deleg:sk1" {
		t.Errorf("expected delegation user id 'deleg:sk1', got %q", got)
	}
}
...
func TestToolRegistry_ExecuteWithContext_EmptyContext(t *testing.T) {
	...
	if got := DelegationUserID(ct.lastCtx); got != "default" {
		t.Errorf("expected default delegation id, got %q", got)
	}
}
