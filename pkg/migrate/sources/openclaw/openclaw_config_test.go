...
func TestLoadOpenClawConfig(t *testing.T) {
	...
	if workspace != "~/.moonhub/workspace" {
		t.Errorf("expected workspace '~/.moonhub/workspace', got '%s'", workspace)
	}
...
func TestConvertToMoonHub(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, warnings, err := cfg.ConvertToMoonHub("")
	if err != nil {
	...
	if picoCfg.Agents.Defaults.Workspace != "~/.moonhub/workspace" {
		t.Errorf("expected workspace '~/.moonhub/workspace', got '%s'", picoCfg.Agents.Defaults.Workspace)
	}
...
func TestConvertToMoonHubWithQQAndDingTalk(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, _, err := cfg.ConvertToMoonHub("")
	if err != nil {
		t.Fatalf("failed to convert config: %v", err)
	}
...
func TestConvertToMoonHubWithMatrix(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
...
func TestConvertToMoonHubWithMatrixDisabled(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
...
func TestToStandardConfig(t *testing.T) {
	...
	if stdCfg.Agents.Defaults.Workspace != "~/.moonhub/workspace" {
		t.Errorf("expected workspace '~/.moonhub/workspace', got '%s'", stdCfg.Agents.Defaults.Workspace)
	}
