...
func TestMatrixLocalpartMentionRegexp(t *testing.T) {
	re := localpartMentionRegexp("moonhub")

	cases := []struct {
		text string
		want bool
	}{
		{text: "@moonhub hello", want: true},
		{text: "hi @moonhub:matrix.org", want: true},
		{
			text: "\u6b22\u8fce\u4e00\u4e0bmoonhub\u5c0f\u9f99\u867e",
...
func TestStripUserMention(t *testing.T) {
	userID := id.UserID("@moonhub:matrix.org")

	cases := []struct {
		in   string
		want string
	}{
		{in: "@moonhub:matrix.org hello", want: "hello"},
		{in: "@moonhub, hello", want: "hello"},
...
func TestIsBotMentioned(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			msg: event.MessageEventContent{
				...
				Mentions: &event.Mentions{
					UserIDs: []id.UserID{id.UserID("@moonhub:matrix.org")},
				},
			},
			want: true,
		},
		...
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/@moonhub:matrix.org">MoonHub</a> hello`,
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/%40moonhub%3Amatrix.org">MoonHub</a> hello`,
			},
			want: true,
		},
	}
...
func TestDownloadMedia_WritesResponseToTempFile(t *testing.T) {
	...
	client, err := mautrix.NewClient(server.URL, id.UserID("@moonhub:matrix.test"), "")
	if err != nil {
