...
// GetMemoryStore returns the memory store for tool access
...
func NewContextBuilder(workspace string) *ContextBuilder {
	...
	builtinSkillsDir := strings.TrimSpace(os.Getenv("MOONHUB_BUILTIN_SKILLS"))
	if builtinSkillsDir == "" {
		wd, _ := os.Getwd()
...
You are moonhub, a helpful AI assistant.
...
%s`, skillsSummary))
	...
	// Learning context (behavioral patterns, preferences, tool usage)
	if cb.memory != nil && cb.memory.HasLearningEngine() {
		learningEngine := cb.memory.GetLearningEngine()
		if learningEngine != nil {
			learningCtx := learningEngine.GetContextString()
			if learningCtx != "" {
				parts = append(parts, learningCtx)
			}
		}
	}
...
func (cb *ContextBuilder) BuildMessages(
	...
	// Add query-based adaptive memory context if available
	// This provides relevant memories based on the current message
	if cb.memory != nil && strings.TrimSpace(currentMessage) != "" {
		adaptiveCtx := cb.memory.GetMemoryContextWithQuery(currentMessage)
		if adaptiveCtx != "" {
			dynamicCtx += "\n\n" + adaptiveCtx
		}

		// Add query-based learning context (behavioral patterns relevant to query)
		if cb.memory.HasLearningEngine() {
			learningEngine := cb.memory.GetLearningEngine()
			if learningEngine != nil {
				// Get learning integration for building query-relevant context
				integration := learning.NewAgentIntegration(
					learningEngine,
					cb.memory.GetAdaptiveMemory(),
					"default",
				)
				learningCtx := integration.BuildLearnedContextForPrompt(currentMessage)
				if learningCtx != "" {
					dynamicCtx += "\n\n" + learningCtx
				}
			}
