...
func NewMigrateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "migrate",
		Short: "Migrate from xxxclaw(openclaw, etc.) to moonhub",
		Args:  cobra.NoArgs,
		Example: `  moonhub migrate
  moonhub migrate --from openclaw
  moonhub migrate --dry-run
  moonhub migrate --refresh
  moonhub migrate --force`,
	...
	cmd.Flags().StringVar(&opts.TargetHome, "target-home", "",
		"Override target home directory (default: ~/.moonhub)")
