	...
	items = append(items,
		MenuItem{
			...
			Action: func() {
				...
				s.addModel(
					moonhubconfig.ModelConfig{ModelName: newName, Model: "openai/gpt-5.4"},
				)
				s.push(
...
func (s *appState) addModel(model moonhubconfig.ModelConfig) {
...
func refreshModelMenu(menu *Menu, currentModel string, models []moonhubconfig.ModelConfig) {
...
func isModelValid(model moonhubconfig.ModelConfig) bool {
...
func (s *appState) testModel(model *moonhubconfig.ModelConfig) {
	if model == nil {
		return
	}
