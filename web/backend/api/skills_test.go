...
func TestHandleListSkills(t *testing.T) {
	...
	oldBuiltin := os.Getenv("MOONHUB_BUILTIN_SKILLS")
	if err := os.Setenv("MOONHUB_BUILTIN_SKILLS", builtinRoot); err != nil {
		t.Fatalf("Setenv(MOONHUB_BUILTIN_SKILLS) error = %v", err)
	}
	defer func() {
		if oldBuiltin == "" {
			_ = os.Unsetenv("MOONHUB_BUILTIN_SKILLS")
		} else {
			_ = os.Setenv("MOONHUB_BUILTIN_SKILLS", oldBuiltin)
		}
	}()
