...
func setupOAuthTestEnv(t *testing.T) (string, func()) {
	...
	oldPicoHome := os.Getenv("MOONHUB_HOME")

	if err := os.Setenv("HOME", tmp); err != nil {
		t.Fatalf("set HOME: %v", err)
	}
	if err := os.Setenv("MOONHUB_HOME", filepath.Join(tmp, ".moonhub")); err != nil {
		t.Fatalf("set MOONHUB_HOME: %v", err)
	}
