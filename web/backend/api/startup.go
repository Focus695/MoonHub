...
const (
	autoStartEntryName = "MoonHubLauncher"
	launchAgentLabel   = "io.moonhub.launcher"
)
...
func linuxAutoStartPath() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".config", "autostart", "moonhub-web.desktop")
}
...
func setLinuxAutoStart(enabled bool, exePath string, args []string) error {
	...
	if enabled {
		...
		content := strings.Join([]string{
			...
			"Version=1.0",
			"Name=MoonHub Web",
			"Comment=Start MoonHub Web on login",
