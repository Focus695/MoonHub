.PHONY: all build install uninstall clean help test ci
...
MOONHUB_HOME?=$(HOME)/.moonhub
WORKSPACE_DIR?=$(MOONHUB_HOME)/workspace
...
## build: Build the moonhub binary for current platform
build: generate
...
## build-launcher: Build the moonhub-launcher (web console) binary
build-launcher:
	@echo "Building moonhub-launcher for $(PLATFORM)/$(ARCH)..."
	@mkdir -p $(BUILD_DIR)
	@if [ ! -f web/backend/dist/index.html ]; then \
		echo "Building frontend..."; \
		cd web/frontend && pnpm install && pnpm build:backend; \
	fi
	@$(GO) build $(GOFLAGS) -o $(BUILD_DIR)/moonhub-launcher-$(PLATFORM)-$(ARCH) ./web/backend
	@ln -sf moonhub-launcher-$(PLATFORM)-$(ARCH) $(BUILD_DIR)/moonhub-launcher
	@echo "Build complete: $(BUILD_DIR)/moonhub-launcher"
...
## build-all: Build moonhub for all platforms
build-all: generate
	@echo "Building for multiple platforms..."
	@mkdir -p $(BUILD_DIR)
...
## install: Install moonhub to system and copy builtin skills
install: build
...
## uninstall: Remove moonhub from system
uninstall:
...
## uninstall-all: Remove moonhub and all data
uninstall-all:
	@echo "Removing workspace and skills..."
	@rm -rf $(MOONHUB_HOME)
	@echo "Removed workspace: $(MOONHUB_HOME)"
...
## ci: Run all CI checks (matches GitHub Actions: build, test, lint)
ci: generate
...
docker-build-full:
	...
	docker compose -f docker/docker-compose.full.yml build moonhub-agent moonhub-gateway
...
docker-run-agent-full:
	docker compose -f docker/docker-compose.full.yml run --rm moonhub-agent
...
help:
	...
	@echo "  WORKSPACE_DIR           # Workspace directory (default: ~/.moonhub/workspace)"
